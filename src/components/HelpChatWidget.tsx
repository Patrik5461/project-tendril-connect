...
export function HelpChatWidget() {
  ...
  const WELCOME: Msg = {
    role: "assistant",
    content: t("helpChat.welcomeMessage"),
  };
  ...
  async function send() {
    ...
    try {
      ...
      if (fnErr) {
        ...
        let msg = t("helpChat.errorConnect");
        try {
      ...
      setError(e?.message ?? t("helpChat.errorGeneric"));
    } finally {
      setBusy(false);
    }
  }
    ...
    <>
      ...
      {open && (
        <div
          ...
          className="fixed bottom-5 left-5 z-40 w-[min(380px,calc(100vw-2.5rem))] h-[min(560px,calc(100vh-2.5rem))] flex flex-col rounded-md bg-background border-2 border-foreground shadow-[6px_6px_0_0_hsl(var(--foreground))] overflow-hidden"
        >
          <div className="flex items-center justify-between border-b-2 border-foreground bg-foreground text-background px-3 py-2">
            ...
            <button
              ...
              aria-label={t("helpChat.closeAria")}
          ...
          >
            ...
            <button
              ...
              aria-label={t("helpChat.sendAria")}
